...
class TextBoxService {
    ...
    describeScene() {
        ...
        text += `\nThere is${room.items.reduce(
            (acc, value, index, items) => `${acc}${index === 0 ? ` ${value.article} ` : index === items.length - 1 ? `, and ${value.article} ` : `, ${value.article} `}${value.name}`, ''
        )}`;
        // TODO: describe what places you are able to travel to
        this.appendTextElement(text, TextElements.paragraph);
    }

    appendTextElement(text: string, elementType: TextElements) {
